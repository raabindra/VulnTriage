...
- **AI-Assisted Analysis (optional, multi-provider, `engines/ai_summary.py`):** an LLM-backed summary added to the PDF report — executive risk overview, per-finding plain-language risk + concrete remediation, and prioritised actions. One request per report with structured JSON output. **Provider auto-selected by which key is set** (Gemini preferred): **`GEMINI_API_KEY`/`GOOGLE_API_KEY`** → Google Gemini via REST (`requests`; default `gemini-flash-latest`, free tier; retries 429/5xx), or **`ANTHROPIC_API_KEY`** → Claude via the `anthropic` SDK (default `claude-haiku-4-5`). Override the model with `AI_SUMMARY_MODEL`. With **no key it is a no-op** and the offline pipeline is unaffected. Enabled by default in all web report paths (Reports endpoint / pipeline / autoscan) and via `cli.py scan … --ai-summary`; threaded as `ai_summary=True` into `ReportGenerator.generate`, rendered by `_ai_section`. Scanner text is passed as data, never instructions; any failure degrades to no section.
